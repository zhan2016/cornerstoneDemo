...
(function (cornerstone) {
    ...
    function getEnabledElement(element) {
        ...
        for(var i=0; i < enabledElements.length; i++) {
            if(enabledElements[i].element === element) { //此处遗漏 使用严格等号 zwj 20170313
...
// This module is a list to operate the StackList zwj 20170307
(function (cornerstone) {
    "use strict";

    var StudyIDStackList = {

    };
    var StackIDList = [];

    function AddStackList(studyID, stacks)
    {
        if(studyID === undefined)
        {
            throw "AddStackList: imageId must not be undefined";
        }
        if(stacks === undefined)
        {
            throw "AddStackList: imagePromise must not be undefined";
        }

        if(StudyIDStackList.hasOwnProperty(studyID) === true) {
            throw "AddStackList: imageId already in cache";
        }

        var StackIDList = {
            "stacks":stacks
        }
        StudyIDStackList[studyID] = StackIDList;
    }

    function GetStackID(studyID)
    {
        if (studyID === undefined)
        {
            throw "GetStackID: studyID must not be undefined";
        }
        var StackList = StudyIDStackList[studyID];
    ...
    function RemoveStackIDList(studyID) {
        if (studyID === undefined)
        {
            throw "RemoveStackIDList: studyID must not be undefined";
        }
        var StackList = StudyIDStackList[studyID];
        //StackList.splice(0, StackList.stacks.length);
        delete StudyIDStackList[studyID];
    }
    cornerstone.StudyIDStackList = {
        AddStackList : AddStackList,
        GetStackID: GetStackID,
        RemoveStackIDList: RemoveStackIDList
    };
...
(function (cornerstone) {
    ...
    function removeImagePromise(imageId) {
        ...
        delete cachedImage.imagePromise; //how to gabrage this imagepromise  change return to delete 20170307
    }
    function removeStudyImagePromise(StudyImageId) {
        if (StudyImageId === undefined)
        {
            throw "removeStudyImagePromise:StudyImageId must not be undefined";
        }
        for (var i = 0, len = StudyImageId.length; i < len; i++)
        {
            for (var j = 0, lenId = StudyImageId[i].imageIds.length; j < lenId; j++)
            {
                removeImagePromise(StudyImageId[i].imageIds[j]);
            }
    ...
    cornerstone.imageCache = {
        ...
        removeStudyImagePromise:removeStudyImagePromise
    };
}(cornerstone));
