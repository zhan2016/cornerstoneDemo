...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    var clickDelay = 50; //修改了点击的延迟时间 张文佳 20170314 最初为200
...
//begin Source:src/imageTools/selectElement.js  张文佳  20170314
(function($, cornerstone,cornerstoneTools){
    'use strict';
    var PreviseData;
    function mouseUpCallback(e, eventData) {
        ...
        //alert('button up');
        if (eventData.element === PreviseData)
        {
            if (cornerstoneTools.SelectIsInList(eventData.element))
            {
                return;
            }

            cornerstoneTools.ClearSelectElement();
            cornerstoneTools.PutAnElement(eventData.element);

            if (cornerstoneTools.referenceLines.synchronizerDataIsNull())
            {
                return;
            }
            var enableElement = cornerstone.getEnabledElement(eventData.element);
            var ImageType = enableElement.image.data.string('x00080008');
            if (ImageType.search(/LOCALIZER/i) === -1)
            {
                cornerstoneTools.referenceLines.ChangeReferenceImageImage(eventData.element);
            }
            else
            {
                cornerstoneTools.referenceLines.ChangeSountImage(eventData.element);
            }
    ...
    cornerstoneTools.SelectElement = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
})($, cornerstone,cornerstoneTools);
//End Source: src/imageTools/selectElement.js

//Begin Source:src/imageTools/SelectList.js 张文佳 20170314
(function($, cornerstone,cornerstoneTools){
    'use strict';
    var SelectedListEnume = [];
    function PutAnElement(e)
    {
        if(e === undefined)
        {
            throw "SelectList: ImageElement must not be undefined";
        }
        e.setAttribute("class", "viewport Data-selected"); //置为选中状态
        SelectedListEnume.push(e);
    }

    function RemoveElement() {
        if(SelectedListEnume.length >= 1) {
            var RemoveElement = SelectedListEnume.pop();
            RemoveElement.setAttribute("class", "viewport"); //置为非选中状态
        }
    ...
    function  ClearSelectElement() {
        if(SelectedListEnume.length >= 1)
        {
            SelectedListEnume.forEach(function (e) {
                e.setAttribute("class", "viewport"); //清楚所有selected的所有元素
            });
            SelectedListEnume = [];
        }

    }
    function GetSelectetEnume() {
        return SelectedListEnume;

    }
    function  SelectIsInList(element) {
        if (SelectedListEnume.indexOf(element) > -1)
        {
            return true;
        }
        else
    ...
    cornerstoneTools.PutAnElement = PutAnElement;
    cornerstoneTools.RemoveElement = RemoveElement;
    cornerstoneTools.ClearSelectElement = ClearSelectElement;
    cornerstoneTools.GetSelectetEnume = GetSelectetEnume;
    cornerstoneTools.SelectIsInList = SelectIsInList;
})($, cornerstone,cornerstoneTools);
//End Source:src/imageTools/SelectList.js 张文佳 20170314
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function defaultStrategy(eventData) {
        ...
        //var text = '(' + x + ',' + y + ')'; //坐標
        var text = '(' + x + ', ' + y + ')'+ '=';
        //var str = 'SP: ' + sp + ' MO: ' + parseFloat(mo.toFixed(3)) + 'HU';
        var str = parseFloat(mo.toFixed(3)) + 'HU';
        if (suv) {
...
//zwj write a new mark to add mark data
// Begin Source: src/imageTools/textMarker.js
(function($, cornerstone, cornerstoneTools) {

    'use strict';

    var toolType = 'textMarkerInput';

    ///////// BEGIN ACTIVE TOOL ///////
    function createNewMeasurement(mouseEventData) {
        var config = cornerstoneTools.textMarkerInput.getConfiguration();
    ...
    cornerstoneTools.textMarkerInput = cornerstoneTools.mouseButtonTool({
        createNewMeasurement: createNewMeasurement,
        onImageRendered: onImageRendered,
        pointNearTool: pointNearTool,
        toolType: toolType
    ...
    cornerstoneTools.textMarkerInputTouch = cornerstoneTools.touchTool({
        createNewMeasurement: createNewMeasurement,
        onImageRendered: onImageRendered,
        pointNearTool: pointNearTool,
        toolType: toolType,
...
// End Source; src/imageTools/textMarkerInput.js
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function addProvider( provider) {
        providers = []; //保证只有一个 providers!  张文佳 20170320
    ...
    function getMetaData(type, imageId) {
        ...
        $.each(providers, function(index, provider) {
            result = cornerstoneTools.provider(type, imageId);
            if (result !== undefined) {
                return true;
            }
        });
...
//Begin Source: src/ProviderOperate.js 张文佳 20170320 写一个工具往provider 里面添加数据
(function($, cornerstone, cornerstoneTools) {

    'use strict';

    // this module defines a way for tools to access ProviderData about an imageId (by parsing DICOM P10).

    var providersData = {};

    function AddProviderData(imageIdValue, Data) {
        //parameters have some errors
        if (imageIdValue === undefined && Data === undefined)
        {
            return;
        }
        //already in list
        if (providersData[imageIdValue] !== undefined)
        {
            return;
        }
        providersData[imageIdValue] = Data;
    }

    function removeAllProviderData( provider) {
        providersData = {};
    }
    ...
    function provider(type, imageId) {
        if (type ===  'imagePlane')
        {
            ...
            if (providersData[imageId] !== undefined)
            {
                return providersData[imageId];
            }
        }
    }
    ...
    // module/private exports
    cornerstoneTools.AddProviderData = AddProviderData;
    cornerstoneTools.removeAllProviderData = removeAllProviderData;
    cornerstoneTools.provider = provider;
    cornerstoneTools.GetProvider = GetProvider;



})($, cornerstone, cornerstoneTools);
//End Source : src/ProviderOperate.js 张文佳 20170320 写一个工具往provider 里面添加数据
...
//Begin Source:src/referenceLines/ReferenceDataManager.js 张文佳 20170321
(function(cornerstoneTools) {

    'use strict';

    // Managere the referenceData
    var synchronizerData={};
    function InitData(synchronizer) {
       if (synchronizer !== undefined)
       {
           synchronizerData["synchronizer"] = synchronizer;
       }
    }
    function  synchronizerDataIsNull() {
        if (synchronizerData === undefined || synchronizerData["synchronizer"] == undefined || synchronizerData["reference"] == undefined)
        {
            return true;
        }
        return false;
    ...
    function AddScount(ScountElement)
    {
        if (ScountElement === undefined)
        {
            return;
        }
        synchronizerData["LOCALIZER"] = ScountElement;
    }
    function  AddReference(ReferenceElement) {
        if (ReferenceElement === undefined)
        {
            return;
        }
        synchronizerData["reference"] = ReferenceElement;
    }

    function ChangeReferenceImageImage(ElementImage)
    {

        if (synchronizerData === undefined || ElementImage == undefined)
        {
            return;
        }
        synchronizerData["synchronizer"].remove(synchronizerData["reference"]);

        var toolData = cornerstoneTools.getToolState(ElementImage, 'stack');
        ...
        //     }
        // );
        cornerstoneTools.addToolState(ElementImage, 'stack', axialStack);
        synchronizerData["synchronizer"].add(ElementImage);
        synchronizerData["reference"] = ElementImage;
    }

    function ChangeSountImage(ElementImage)
    {
        if (synchronizerData === undefined || ElementImage == undefined)
        {
            return;
        }
        synchronizerData["synchronizer"].remove(synchronizerData["LOCALIZER"]);
        synchronizerData["synchronizer"].add(ElementImage);
        synchronizerData["LOCALIZER"] = ElementImage;
    }


    // module/private exports
    cornerstoneTools.referenceLines.InitData = InitData;
    cornerstoneTools.referenceLines.AddReference = AddReference;
    cornerstoneTools.referenceLines.ChangeReferenceImageImage = ChangeReferenceImageImage;
    cornerstoneTools.referenceLines.ChangeSountImage = ChangeSountImage;
    cornerstoneTools.referenceLines.AddScount = AddScount;
    cornerstoneTools.referenceLines.synchronizerDataIsNull = synchronizerDataIsNull;

})(cornerstoneTools);

//End Source:src/referenceLines/ReferenceDataManager.js 张文佳 20170321
...
(function(cornerstone, cornerstoneTools) {
    ...
    function newStackSpecificToolStateManager(toolTypes, oldStateManager) {
        ...
        //恢復堆棧到初始狀態 張文佳 20170315
        function  RestoreAll(element) {
            oldStateManager.clear(element);
